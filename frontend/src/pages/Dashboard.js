  ...
  for (const symbol of symbols) {
    ...
    if (!y) {
      ...
      if (t.combined_oi !== y.combined_oi) {
        diffs.push(
          `OI ${formatNumber(y.combined_oi)} → ${formatNumber(t.combined_oi)}`
        );
      }
    ...
    <table className="market-table">
      ...
      <tbody>
        {data.map((row) => {
          ...
          const hasBreakdown =
            row.series_breakdown &&
            Object.keys(row.series_breakdown).length > 0;

          return (
            <React.Fragment key={row.symbol}>
              <tr
                onClick={() => hasBreakdown && toggleRow(row.symbol)}
                className={`${hasBreakdown ? "expandable" : ""} ${isExpanded ? "row-expanded" : ""}`.trim()}
              >
                <td>
                ...
                <td
                  className={
                    row.risk_status === "MANDATORY FORECLOSURE"
                      ? "risk-foreclosure"
                      : ""
                  }
                >
                ...
                <tr className="breakdown-row">
                  <td colSpan="6">
                    <div className="breakdown-inner">
                      <h4 className="breakdown-title">Series Breakdown</h4>
                      <table className="breakdown-table">
                        <tbody>
                          {Object.entries(row.series_breakdown).map(
                            ([series, val]) => (
                              <tr key={series}>
                                <td>Series {series}:</td>
      ...
      <tbody>
        {changes.map((row) => (
          <tr key={row.symbol}>
            ...
            <td
              className={
                row.detail === "Added" || row.detail === "Removed"
                  ? "change-label"
                  : "change-detail"
              }
            >
